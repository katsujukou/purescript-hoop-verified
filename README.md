...
For instance, the following snippet showcases a typical F\* implementation. Here, the `snoc` function (which appends an element to the end of a list) specifies that it returns a list whose length is precisely incremented by one via a *refinement type*. Additionally, the lemma `rev_preserves_length` independently states and *proves* as its own that the list reversal function `rev` preserves the original length of the list:
