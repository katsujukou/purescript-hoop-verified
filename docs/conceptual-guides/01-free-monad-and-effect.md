...
> This series aims to explain the ideas behind Hoop's implementation rather than provide a rigorous mathematical introduction. The discussion below is therefore intentionally informal.
...
Since both are Monoids, they naturally satisfy associativity and the identity laws. In other words, the monoid laws alone do not determine which operation is being used. What differentiates them is **the additional structural properties** they possess on top of the monoid laws.
...
> More precisely, the free monoid on a set \(X\) is characterized by a universal property involving the forgetful functor
> $$
> U : \mathbf{Mon} \to \mathbf{Set},
> $$
> which maps each monoid to its underlying set and each monoid homomorphism to its underlying function.
>
> For every monoid $M$ and every function $f : X \to U(M)$, there is a unique monoid homomorphism
> $$
> \bar f : F(X) \to M
> $$
> whose restriction to the generators is $f$. Equivalently, the free-monoid functor $F : \mathbf{Set} \to \mathbf{Mon}$ is left adjoint to $U$.
>
> For further details, see Bartosz Milewski’s [“Free Monoids”](https://bartoszmilewski.com/2015/07/21/free-monoids/) in *Category Theory for Programmers*.
...
Because a Free Monad does not perform any lossy folding and instead incrementally accumulates each sub-expression into a data structure, this do block isn't actually performing an active calculation. Instead, it **generates a piece of data** which conceptually looks like something as follows:
...
## Other Represenation of Free Monad
...
Sometimes this encoding is called operational or *freer* representation because it does not require even functoriality of `f`. This operational representation is essentially `Free (Coyoneda f)`. If `f` is already a functor, `Coyoneda f` is naturally isomorphic to `f`, relating this representation back to the naive `Free f`.
