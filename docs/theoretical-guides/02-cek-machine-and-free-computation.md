# CEK-machine and Interpreting of Free Computation
...
- **Environment**: A mapping from actions to their corresponding handling clauses, alongside references to mutable cells introduced by `var` (which we will cover in detail in later chapters)
...
This definition is a significantly simplified version of the actual AST type defined in `Hoop.Runtime.Syntax.fst`, tailored specifically for the conceptual explanations in this section.
...
> [Birthe van den Berg and Tom Schrijvers: *A Framework for Higher-Order Effects & Handlers*](https://arxiv.org)
...
Compared to the `comp_tree` introduced in the previous section, a new type parameter `cl` has been added. This parameter represents the type of the clauses. Since clauses in the Hoop runtime are type-erased, opaque JavaScript closures, they are squashed into an opaque type parameter. Furthermore, due to the extensions in this section, an AST node can now wrap stack frames, and a stack frame can wrap an AST, meaning both types are now consolidated into a mutually recursive group.
...
A *prompt* serves as a landmark for the machine to non-sequentially transfer the control flow to the handler's location when an effect action is performed. This mechanism can be clearly observed in the state transition table for the `Perform` node below (where `@` denotes the splitting/concatenation of stack lists):

### Performing

| Current C | Current K | Next C | Next K |
| :--- | :--- | :--- | :--- |
| `Perform eff op pay` | $\kappa_1$ @ [`PromptF hs`] @ $\kappa_2$ <br> where<br> `hs = { ..., [eff]: {..., [op]: h } }` | `h(...pay, kont)`<br>where<br>`kont = fun v -> Resumed κ_2 v` | $\kappa_2$ |
...
> *Note*: A bit of terminology: the value accepted by a continuation is commonly referred to as the **answer**.

Here, `b` represents the answer type, and `o` is the output type returned to the perform site upon resumption. On the runtime side, as mentioned earlier, a continuation is merely a lambda that constructs and returns a `Resumed` node. The true nature of `continue` is simply to reveal the underlying lambda type `b -> Hoop effrow o` from the opaque `Cont b effrow o` type. Once a `Resumed` node is loaded into the C-register via resumption, the machine redirects the execution back to the `perform` site according to the following transition table:

| Current C | Current K | Next C | Next K |
|---|---|---|---|
| `Resumed κ' v` | $\kappa$ | `Var v` | $\kappa^\prime$ @ $\kappa$ |

As shown above, processing a `Resumed` node simply prepends the captured continuation stack $\kappa^\prime$ back onto the current stack $\kappa$, wraps the value passed to continue into a `Var` node, and loads it into the C-register.
...
> ```
>
> While the paper's `Op` directly couples a concrete effect signature `σ` with its continuation, Hoop’s `Op` adopts a variation of the Codensity / Yoneda-transformed Free monad (or a Church-encoded Free monad) introduced in the previous chapter.
>
...
>
> This architectural choice enables the Hoop runtime (CEK machine) to avoid deeply deconstructing effect data structures. The moment it encounters an `Op` node, it executes an fast stack transition—setting the left-hand side into the Control register and pushing the right-hand side onto the Kontinuation stack -- all while ensuring that Z3 can easily track and verify the proofs without getting lost.

## Deep vs Shallow Handler Semantics

In the state transition rule for the `Perform` node presented in the previous section, the captured continuation supplied to the clause was defined as $\kappa_1$. Note that the target frame `PromptF` hs itself was dropped from the resulting continuation context $\kappa_2$, meaning the handler is discarded from the stack during traversal.
Interestingly, whether a captured continuation retains or discards its handling prompt is the exact deciding factor that distinguishes **Deep and Shallow handler semantics**.
...
By capturing the stack frames strictly up to $\kappa_2$ and isolating the execution context, Hoop's transition table forms the foundation for verifiable, highly performant deep-handler semantics.
